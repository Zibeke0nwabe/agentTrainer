...
function renderDepartmentCards(containerId) {
  ...
  // Determine which page we're on: /home, /chat, etc.
  const currentPage = window.location.pathname;

  // Logic to update links based on the page route
  if (currentPage === '/home') {
    departments.forEach(dept => {
      dept.chatLink = "/chat";  // Update Chat App link for /home
      dept.callLink = "/voice"; // Update Voice Call link for /home
    });
  } else if (currentPage === '/') {
    departments.forEach(dept => {
      dept.chatLink = "/login"; // for users that are not logged in
      dept.callLink = "/login";   // for users that are not logged in
    });
  }
  }
